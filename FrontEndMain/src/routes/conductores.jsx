...
function createData(id_conductor, nombre, estado, categogoria, actualizacion) {
  return { id_conductor, nombre, estado, categogoria, actualizacion };
}
      ...
      {heads.map((head) => {
        ...
        return (
          <StyledTableCell key={head.id} style={{ minWidth: head.minWidth }} className={`head_${head.key}`}>
            {
              (head.key === "estado") ?
                (<div className="conductoresBlock_tableStates">
                  {row.estado.map((item) => (
                    <div className={`conductoresBlock_tableState conductoresBlock_tableState--${item}`}>
                      <p>{item}</p>
                    </div>
                  ))}
                </div>) :
...
export default function Conductores() {
  return (
    <>
      <div className="conductoresBlock">
        <div className="conductoresBlock_tittle">
          <h2>Conductores</h2>
        </div>
        <div className="conductoresBlock_options">
          <button className="conductoresBlock_button">
        ...
        <Paper
          sx={{
            ...
            overflow: 'hidden',
            padding: "12px 16px",
            '& .head_nombre': { color: 'rgba(79, 115, 150, 1)' },
            '& .head_categogoria': { color: 'rgba(79, 115, 150, 1)' },
            '& .head_actualizacion': { color: 'rgba(79, 115, 150, 1)' },
          }}
          elevation={0}
        >
          <TableVirtuoso
