...
for (let i = 0; i < 2000; i++) {
  rows.push(createData(867686, 13, "Conductor", ["Activo"], "Lima", "1m"));
}

function createData(id_orden, ruta, conductor, estado, creada, finalizada) {
  return { id_orden, ruta, conductor, estado, creada, finalizada };
}
      ...
      {heads.map((head) => {
        ...
        return (
          <StyledTableCell key={head.id} style={{ minWidth: head.minWidth }} className={`head_${head.key}`}>
            {
              (head.key === "estado") ?
                (<div className="odenesBlock_tableStates">
                  {row.estado.map((item) => (
                    <div className={`odenesBlock_tableState odenesBlock_tableState--${item}`}>
                      <p>{item}</p>
                    </div>
                  ))}
                </div>) :
...
export default function Ordenes() {
  return (
    <>
      <div className="odenesBlock">
        <div className="odenesBlock_tittle">
          <h2>Ordenes</h2>
        </div>
        <div className="odenesBlock_options">            
            <Stack spacing={2} sx={{ width: 300 }}>
            <Autocomplete
                freeSolo
                id="search-box"
                disableClearable
                options={List_ordenes.map((option) => option.title)}
                renderInput={(params) => (
                <TextField
                    {...params}
                    label="Buscar Orden"
        ...
        <Paper
          sx={{
            ...
            overflow: 'hidden',
            padding: "12px 16px",
            '& .head_ruta': { color: 'rgba(79, 115, 150, 1)' },
            '& .head_conductor': { color: 'rgba(79, 115, 150, 1)' },
            '& .head_creada': { color: 'rgba(79, 115, 150, 1)' },
            '& .head_finalizada': { color: 'rgba(79, 115, 150, 1)' },
          }}
          elevation={0}
        >
          <TableVirtuoso
